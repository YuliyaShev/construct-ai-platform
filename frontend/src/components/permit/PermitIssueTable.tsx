...
export function PermitIssueTable({ issues }: { issues: Issue[] }) {
  return (
        ...
        <TBody>
          ...
          {issues.map((iss) => (
            <TR key={iss.id}>
              ...
              <TD className="text-sm text-slate-700 dark:text-slate-200">{iss.recommendation || "—"}</TD>
            </TR>
          ))}
        </TBody>
      </Table>
